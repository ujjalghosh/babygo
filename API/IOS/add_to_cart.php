...
//if (isset($_POST['add_to_wishlist']))  {
 if (array_key_exists('add_to_cart', $obj)) {
...
if(!empty($aa)){
  ...
  $order_status       =   "Cart";
  $orderchk_query = $db->query("select * from " . $db->tbl_pre . "orders_tbl where customer_id='" . $customer_id . "' and order_status='Cart' and product_id='".rep($product_id)."' ", PDO::FETCH_BOTH);
...
$order_num = $db->total($orderchk_query);
 ...
 if (!empty($product_id) && !empty($customer_id) && !empty($product_details_id) && !empty($set) && !empty($mrp)&& !empty($piece) && !empty($amount) ) {
  $product = array();
$name_value = array('product_id' => rep($product_id), 'customer_id' => rep($customer_id), 'product_details_id' => rep($product_details_id), 'total_set' =>  $set, 'mrp' => rep($mrp), 'piece'=>rep($piece),'amount'=>rep($amount), 'order_status'=>rep($order_status));
  ...
  $order_add = $db->insert('orders_tbl', $name_value);
   ...
   $response["status"] = FALSE;
        $response["msg"] = "Product already exist in your cart.";
}
