...
if (isset($_REQUEST['product_search'])) {
    $search_term =$_REQUEST['product_search'];
}else{  $search_term ='';}
...
elseif ((isset($_REQUEST['sort_by'])) && !isset($_REQUEST['refine_by'])) {
  ...
  $product_array = $Product->product_display($db->tbl_pre . "product_tbl", array(), " WHERE (style_no like '%".$search_term."%') or (product_name like '%".$search_term."%')  order by product_id DESC limit ".$position.", 4 " );   
	}
