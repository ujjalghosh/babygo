...
$(document).ready(function() {
	...
	//$("#results").load("ajax/product_list.php");  //initial page number to load
		   $.ajax({
           type: "POST",
           url: 'ajax/product_search.php',          
          data: $('#product_search').serialize() , // serializes the form's elements.       
           success: function(data)
           {
           	$("#results").html(data);
	...
	$(".pagination").bootpag({
	   ...
	   maxVisible: 5 
		...
		e.preventDefault(); 
		var data= $('#product_search').serialize() + "&page=" + num;
