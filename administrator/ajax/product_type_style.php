...
$aColumns = array('ut.style_id','ut.product_type', 'ut.product_style', 'ut.product_for','ut.product_style_status' );
$sIndexColumn = "ut.style_id";
$sTable = $db->tbl_pre . "product_type_style_tbl ut";
$sWhere = "WHERE ut.style_id<>0";
//print_r($_GET);
//// Pagination //
$sLimit = "";
...
$sQuery = $db->query("SELECT * FROM " . $db->tbl_pre . "product_type_style_tbl");
...
for ($j = 0; $j < count($aRow); $j++) {
	$row = array();
	for ($i = 0; $i < count($aColumns); $i++) {
		if ($aColumns[$i] != ' ' && $aColumns[$i] != 'ut.product_type_status') {
			if ($aColumns[$i] == 'ut.franchise_password') {
				$row[] = decode($aRow[$j][$i]);
			} else {
				$row[] = $aRow[$j][$i];
			}
		} elseif ($aColumns[$i] != ' ' && $aColumns[$i] == 'ut.product_type_status') {
